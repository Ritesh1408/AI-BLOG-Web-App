...
const BlogPostEditor = ({ isEdit }) => {
  ...
  const [openBlogPostGenForm, setOpenBlogPostForm] = useState({
    open: false,
    data: null,
  });
  ...
  const generatePostIdeas = async() => {
    setIdeaLoading(true);
    try {
      const aiResponse = await axiosInstance.post(API_PATHS.AI.GENERATE_BLOG_POST_IDEAS,
        {
  ...
  const handlePublish = async(isDraft) => {
    ...
    try {
      ...
      const response = isEdit ? await axiosInstance.put(API_PATHS.POSTS.UPDATE(postData.id),
        reqPayload
        )
  ...
  return (
    <DashboardLayout activeMenu="Blog Post">
                  ...
                  <div data-color-mode="light" className='mt-3'>
                    <MDEditor 
                      ...
                      commands={[
                        ...
                        commands.hr,
                        commands.heading, // title is depreceated
                        commands.divider,
                        commands.link,
                        commands.code,
                        commands.image,
            ...
            <div className='form-card col-span-12 md:col-span-4 p-0'>
              <div className='flex items-center justify-between px-6 pt-6'>
                ...
                <button
                  ...
                  onClick={() => setOpenBlogPostForm({ open: true, data: null })}
                >
                  Generate New
                </button>
              </div>
                ...
                {ideaLoading ? (
                  ...
                  postIdeas.map((idea, index) => (
                    <BlogPostCard 
                      ...
                      onSelect={() => setOpenBlogPostForm({ open: true, data: idea })}
                    />
                  ))
                )}
              </div>
      ...
      <Modal
        ...
        onClose={() => {
          setOpenBlogPostForm({ open:false, data:null })
        }}
        hideHeader
      >
